...
function base_system()
{
	...
	if [ ! -d "/documentation" ]; then
		...
		# running in vagrant
		bundle install --gemfile="/documentation/Gemfile"
	fi
...
function jekyll_hook()
{
	...
	echo "    \"public_repo\": false," >> /root/jekyll-hook/config.json
	...
	echo "      }" >> /root/jekyll-hook/config.json
	echo "    }," >> /root/jekyll-hook/config.json
	echo "    \"secret\": \"abc123\"," >> /root/jekyll-hook/config.json
	echo "    \"email\": {" >> /root/jekyll-hook/config.json
	echo "        \"isActivated\": true," >> /root/jekyll-hook/config.json
	echo "        \"user\": \"townsuitedocsite@gmail.com\"," >> /root/jekyll-hook/config.json
	echo "        \"password\": \"ADFgafd234656767fgfgAFSGjfgip\"," >> /root/jekyll-hook/config.json
	echo "        \"host\": \"smtp.gmail.com\"," >> /root/jekyll-hook/config.json
	echo "        \"ssl\": true" >> /root/jekyll-hook/config.json
	echo "    }," >> /root/jekyll-hook/config.json
	echo "    \"accounts\": [" >> /root/jekyll-hook/config.json
	echo "        \"TownSuite\"" >> /root/jekyll-hook/config.json
	echo "    ]" >> /root/jekyll-hook/config.json
	echo "}" >> /root/jekyll-hook/config.json
